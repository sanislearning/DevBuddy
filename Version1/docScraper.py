#This one keeps running into a singular bug, checked manually, checked via AI, can't fix it rn
#Something about the response lacking a status_code field, printed the entire response body out and realised that the LLM is unable to extract info
#Probably an issue with the way the model tries to structure the outputs
...
async def extract_structured_data_using_llm(
    ...
    # Convert Pydantic model to JSON schema for serialization
    json_schema = InfoConsume.schema()
    ...
    for res in results:
        if res.extracted_content:
            markdown_output += json.dumps(res.extracted_content, indent=2)
            markdown_output += "\n\n"
        else:
            print(f"Failed extraction. Full result object:\n{res}")
